...
resstock_yearly_with_metadata_weather.write.saveAsTable("building_model.resstock_yearly_with_metadata_weather")

# COMMAND ----------

## lets read the data

resstock_yearly_with_metadata_weather = spark.table('building_model.resstock_yearly_with_metadata_weather')
...
resstock_yearly_with_metadata_weather = (resstock_yearly_with_metadata_weather.where(F.col('upgrade_id') == 0))
...
target_variable = 'sum_out_natural_gas_heating_total'
...
#         'in_window_wall_ratio_mean', 'in_bedrooms', 'in_geometry_stories','in_ducts_insulation', 'in_ashrae_iecc_climate_zone_2004','in_income_bin_midpoint', 'in_is_vacant']
...
covariates = ['in_occupants', 'temp_high', 'temp_low', 'temp_avg',
       ...
       'std_temp_low', 'std_wind_speed', 'std_ghi', 'in_vintage', 'in_sqft', 'in_hvac_heating_efficiency_nominal_percent', 'in_infiltration_ach50',
        ...
        'in_hvac_cooling_type', 'in_hvac_cooling_efficiency', 'in_hvac_cooling_partial_space_conditioning', 'in_is_vacant', 'in_is_rented',
        'in_ducts_insulation', 'in_hvac_has_ducts', 'in_ducts_leakage']
...
grouping_variable = 'in_ducts_leakage'
...
#actuals_and_preds =actuals_and_preds[((actuals_and_preds['Actual'] <  18754.36840))]

wMAPE = np.mean(actuals_and_preds.abs_error)/np.mean(actuals_and_preds.Actual+10)
WMPE =  np.mean(actuals_and_preds.error)/np.mean(actuals_and_preds.Actual+10)
print([wMAPE, WMPE])

# COMMAND ----------

#np.median(actuals_and_preds["abs_error"]/actuals_and_preds["Actual"])
quantiles = actuals_and_preds["abs_error"].quantile([.01,.05,.1,0.25, 0.5, 0.75, .9, .95, .99])/np.median(actuals_and_preds.Actual)
quantiles

# COMMAND ----------

#actuals_and_preds =actuals_and_preds[((actuals_and_preds['Actual'] <  np.quantile(actuals_and_preds['Actual'], q =.99)))]
np.quantile(actuals_and_preds["abs_error"]/(actuals_and_preds["Actual"] + 10), q=[.01,.05,.1,0.25, 0.5, 0.75, .9, .95, .99])
#np.quantile(actuals_and_preds["Predicted"], q=[0,.01,.05,.1,0.25, 0.5, 0.75, .9, .95, .99, 1])


# COMMAND ----------

np.corrcoef(actuals_and_preds["Actual"], actuals_and_preds["Predicted"])
