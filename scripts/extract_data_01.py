...
RESSTOCK_PATH = CUBE_RAW_DATA_PATH / "nrel" / "end_use_load_profiles" / "2022" / "resstock_tmy3_release_1"

BUILDING_METADATA_2022_PARQUET_PATH = str(
    RESSTOCK_PATH / "metadata_and_annual_results" / "national" / "parquet" / "baseline_metadata_only.parquet"
)

BUILDING_METADATA_2024_PARQUET_PATH = str(CUBE_RAW_DATA_PATH / "bsb_sims" / "nrel_2024_2_baseline" / "baseline.parquet")
...
def align_baseline_metadata(
    building_metadata_2022: DataFrame, building_metadata_2024: DataFrame, verbose: bool = True
) -> DataFrame:
    """
    ...
    drop_columns_2022 = ["hvac_secondary_heating_type_and_fuel", "schedules"]
    ...
    building_metadata_2022 = building_metadata_2022.withColumn(
        "duct_leakage_and_insulation", F.regexp_replace("ducts", "Leakage", "Leakage to Outside")
    ).drop("ducts")
    ...
    building_metadata_2022 = building_metadata_2022.withColumn(
        ...
        "water_heater_location", F.when(F.col("water_heater_in_unit") == "Yes", "Inside Unit").otherwise("Outside")
    )
    ...
    building_metadata_2024 = (
        building_metadata_2024.withColumn(
            "hvac_heating_type_and_fuel", F.regexp_replace("hvac_heating_type_and_fuel", "MSHP", "ASHP")
        )
    ...
    building_metadata_2022 = building_metadata_2022.withColumn(
        "hvac_cooling_type",
        F.when(F.col("hvac_cooling_type") == "Heat Pump", "Ducted Heat Pump").otherwise(F.col("hvac_cooling_type")),
    )

    if verbose:
        print(
            "Columns in 2022 and not 2024:",
            set(building_metadata_2022.columns).difference(building_metadata_2024.columns),
        )
        print(
            "Columns in 2024 and not 2022:",
...
building_metadata_2022 = feature_utils.clean_building_metadata(
...
building_metadata_2024 = feature_utils.clean_building_metadata(
...
annual_outputs = annual_rastock_outputs.unionByName(annual_resstock_outputs, allowMissingColumns=True)

# cast pkeys to the right type since longs cannot be pkeys in feature tables
annual_outputs = annual_outputs.withColumn("building_id", F.col("building_id").cast("int")).withColumn(
    "upgrade_id", F.col("upgrade_id").cast("double")
)
...
def extract_hourly_weather_data() -> DataFrame:
    ...
    weather_data = (
        ...
        .withColumn("county_gisjoin", F.element_at(F.split(F.input_file_name(), "/|_"), -2))
        ...
        .withColumn("datetime_formatted", F.date_format(F.col("date_time"), "MM-dd-HH:00"))
