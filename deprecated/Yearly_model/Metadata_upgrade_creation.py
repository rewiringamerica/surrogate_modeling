...
metadata = metadata.filter(
    col("in_geometry_building_type_acs").isin(eligible_households)
)
...
metadata.loc[met_conditions, "in_hvac_backup_heating_efficiency_nominal_percent"] = (
    "100%"
)
...
def attic_insulation_IECC_CZ1A(df):
    met_conditions = (
        (df["in_ashrae_iecc_climate_zone_2004"] == "1A")
...
def attic_insulation_IECC_CZ2A_2B_3A_3B_3C(df):
    met_conditions = (
        (df["in_ashrae_iecc_climate_zone_2004"].isin(["2A", "2B", "3A", "3B", "3C"]))
        & (df["in_geometry_attic_type"] == "Vented Attic")
        & (
            df["in_insulation_ceiling"].isin(
                ["Uninsulated", "R-7", "R-13", "R-19", "R-30"]
            )
        )
    )
    if met_conditions.any():
...
def attic_insulation_IECC_CZ4A_7C(df):
    met_conditions = (
        (
            df["in_ashrae_iecc_climate_zone_2004"].isin(
        ...
        & (
            df["in_insulation_ceiling"].isin(
                ["Uninsulated", "R-7", "R-13", "R-19", "R-30", "R-38"]
            )
        )
    )
    if met_conditions.any():
...
def apply_upgrade_foundation_wall_insulation(df):
    met_conditions = (
        df["in_geometry_foundation_type"].isin(
...
def apply_upgrade_rim_joist_insulation(df):
    met_conditions = (
        df["in_geometry_foundation_type"].isin(
            ["Unvented Crawlspace", "Vented Crawlspace", "Heated Basement"]
        )
    ) & (df["in_insulation_foundation_wall"] == "Uninsulated")
    if met_conditions.any():
...
def apply_upgrade_roof_insulation_to_R30(df):
    met_conditions = (
        df["in_geometry_attic_type"] == "Finished Attic or Cathedral Ceilings"
    ) & (
        df["in_insulation_roof"].isin(
            ["Finished, Uninsulated", "Finished, R-7", "Finished, R-13"]
        )
    )
    if met_conditions.any():
...
def apply_upgrade_03(df):
    ...
    apply_logic_asHP = (df["in_hvac_has_ducts"] == True) & (
        ...
        | df["in_hvac_heating_efficiency"].isin(
            [
    ...
    df.loc[apply_logic_msHP, "in_hvac_heating_efficiency"] = (
    ...
    df.loc[
        apply_logic_asHP | apply_logic_msHP,
...
def apply_upgrade_04(df):
    ...
    apply_logic_nonducted_msHP = (df["in_hvac_has_ducts"] == False) & (
        ~df["in_hvac_cooling_type"].str.contains("Heat Pump", na=False)
        | df["in_hvac_heating_efficiency"].isin(
            ["MSHP, SEER 14.5, 8.2 HSPF", "MSHP, SEER 29.3, 14 HSPF, Max Load"]
        )
    )

    df.loc[apply_logic_ducted_msHP, "in_hvac_heating_efficiency"] = (
        "MSHP, SEER 24, 13 HSPF"
    )

    df.loc[apply_logic_nonducted_msHP, "in_hvac_heating_efficiency"] = (
        "MSHP, SEER 29.3, 14 HSPF, Max Load"
    )

    df.loc[
        apply_logic_ducted_msHP | apply_logic_nonducted_msHP, "eligible_for_upgrade"
    ] = 1
    df.loc[
        apply_logic_ducted_msHP | apply_logic_nonducted_msHP, "in_hvac_cooling_type"
    ] = "Heat Pump"
    df.loc[
        apply_logic_ducted_msHP | apply_logic_nonducted_msHP,
        "in_hvac_cooling_partial_space_conditioning",
    ] = "100%"
    df.loc[apply_logic_ducted_msHP | apply_logic_nonducted_msHP, "in_heating_fuel"] = (
        "Electricity"
    )
    df.loc[
        apply_logic_ducted_msHP | apply_logic_nonducted_msHP,
...
def apply_upgrade_05(df):
    ...
    df.loc[apply_logic_asHP, "in_hvac_backup_heating_efficiency_nominal_percent"] = (
        "100%"
    )
    df.loc[apply_logic_asHP, "in_hvac_heating_efficiency"] = "ASHP, SEER 15, 9.0 HSPF"

    df.loc[apply_logic_else, "in_hvac_backup_heating_efficiency_nominal_percent"] = (
        df.loc[apply_logic_else, "in_hvac_heating_efficiency_nominal_percent"]
    )
...
metadata_w_upgrades["in_vintage"] = metadata_w_upgrades["in_vintage"].apply(
    util_datagen.vintage2age2010
)

metadata_w_upgrades["in_ducts_leakage"] = metadata_w_upgrades[
    "in_ducts_leakage"
].fillna(0)

metadata_w_upgrades["in_geometry_stories"] = metadata_w_upgrades[
    "in_geometry_stories"
].astype(float)

metadata_w_upgrades["in_hvac_heating_efficiency_nominal_percent"] = metadata_w_upgrades[
    "in_hvac_heating_efficiency"
].apply(util_datagen.convert_heating_efficiency)

metadata_w_upgrades["in_hvac_seer_rating"] = metadata_w_upgrades[
    "in_hvac_heating_efficiency"
].apply(util_datagen.extract_seer)

metadata_w_upgrades["in_hvac_hspf_rating"] = metadata_w_upgrades[
    "in_hvac_heating_efficiency"
].apply(util_datagen.extract_hspf)

metadata_w_upgrades["in_hvac_afue_rating"] = metadata_w_upgrades[
    "in_hvac_heating_efficiency"
].apply(util_datagen.extract_afue)


# COMMAND ----------

## obtain cooling efficiency
met_conditions = metadata_w_upgrades["in_hvac_cooling_type"].str.contains(
    "Heat Pump", na=False
)
metadata_w_upgrades.loc[met_conditions, "in_hvac_cooling_efficiency"] = (
    "SEER "
    + " "
    + metadata_w_upgrades.loc[met_conditions, "in_hvac_seer_rating"].astype(str)
)

metadata_w_upgrades["in_hvac_cooling_efficiency"] = metadata_w_upgrades[
    "in_hvac_cooling_efficiency"
].apply(util_datagen.extract_cooling_efficiency)
