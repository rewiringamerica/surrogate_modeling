...
weather_data = (
    weather_data.withColumn("below_32", F.when(F.col("temp_air") < 32, 1).otherwise(0))
    .withColumn("below_41", F.when(F.col("temp_air") < 41, 1).otherwise(0))
    .withColumn(
...
def WeatherAggregation(
    ...
    weather_full_daily = (
        (
            weather_data.join(
                weather_metadata.select("canonical_epw_filename", "county_geoid"),
                on="canonical_epw_filename",
            )
        )
        .groupBy("day", "month", "county_geoid")
        .agg(
            F.max(col("temp_air")).alias("temp_high"),
            F.min(col("temp_air")).alias("temp_low"),
            F.avg(col("temp_air")).alias("temp_avg"),
            F.avg(col("wind_speed")).alias("wind_speed_avg"),
    ...
    if aggregation_level == "monthly":
        weather_full_monthly = (
            (weather_full_daily)
            .groupBy("month", "county_geoid")
            .agg(
                F.avg(col("temp_high")).alias("temp_high"),
                F.avg(col("temp_low")).alias("temp_low"),
                F.avg(col("temp_avg")).alias("temp_avg"),
                F.avg(col("wind_speed_avg")).alias("wind_speed_avg"),
...
weather_data_full = WeatherAggregation(
    weather_data=weather_data,
    weather_metadata=weather_metadata,
